...
// TestEachContainer_MatchesNaive exercises the 32-bit primitive directly.
func TestEachContainer_MatchesNaive(t *testing.T) {
	// only exercised indirectly via EachPage above for the 64-bit type; here we
	// confirm a single page round-trips through EachPage for a large id.
	assertEachPageMatches(t, (7<<32)|(9*pageSize)|123, (7<<32)|(9*pageSize)|124)
}
